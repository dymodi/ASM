...
// Here we only consider the situation that row >= col
...
void gs_m(float *A, int m, int n, float *Q, float *R) {
	...
	for (i = 0; i < n; i++) {
		...
		for (j = 0; j < m; j++) {
			Q[j*n+i] = A[j*n+i]/en;
			printf("en:%f\n",en);
			printf("j:%d, i:%d, A[j*n+i]:%f\n",j, i, A[j*n+i]);
			printf("j:%d, i:%d, Q[j*n+i]:%f\n",j, i, Q[j*n+i]);
		}
					...
					if ((addBoundConstraint == 1) && (addGeneralConstraint == 0)) {					
						...
						if (ml == 1) {
							...
							//if (needCheckFlag == 1) {	// Imprecision may happen
                                ...
                                if (maxDiff > 0.001) {		    // Threshold need to be set
									// Update Error, initialize factorization									
                                    //Print("maxDiff: %f. No ReUpdate.\n",maxDiff);
                                    // Get Av
									Print("maxDiff: %f. ReUpdate here.\n",maxDiff);
									for (i = 0; i < ml; i++) {
										for (j = 0; j < ndec; j++) 
											tmpVec1[j] = AA[(wl[i]-1)*ndec+j];		// tmpVec1 = AA(wl(i),:);
										permut(tmpVec1, orderPermu, ndec, 1,tmpMat6);
...
// 						vec_sub(tmpMat1,tmpMat3,nv*ml,tmpMat4);
// 						for (i = 0; i < nv*ml; i++) {
// 							if (tmpMat4[i] > 0.0005 || tmpMat4[i] < -0.0005) {
